...
│   │       └── memory_service.py      # ChromaDB 三层记忆（会话/Agent/项目）
│   ├── agent_service/
│   │   ├── runners/
...
| **中期** | Agent 级别 | 该 Agent 的历史经验、常见错误模式、设计偏好 |
...
| POST | `/api/projects/{id}/tasks` | 创建任务并触发流水线 `{title, description?, agent_id}` |
